...
# will need to create a new vector that steps through the data and then do a join
...
Temp_timeseries <- Temp_BR_processed %>%
  ggplot(aes(
    x = Date,
    y = Temperature_C
  )) +
...
Rainfall_timeseries <- Rainfall_BR_processed %>%
  ggplot(aes(
    x = Date,
    y = Rainfall
  )) +
...
Rainfall_ts <- ts(Rainfall_BR_processed$Rainfall, start = c(1900,1), frequency = 12) 

Temperature_ts <- ts(Temp_BR_processed$Temperature_C, start = c(1900, 1), frequency = 12)
```
...
Rainfalldecomposed <- stl(Rainfall_ts, s.window = "periodic")
...
Rainfall_components <- as.data.frame(Rainfalldecomposed$timeseries[,1:3])
