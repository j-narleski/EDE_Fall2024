...
> Add notes here: We might be interested in long-term observations of temperature, oxygen, and light in lakes because these can be indicative of long-term environmental health changes to the lake, especially if there's evidence of eutriphocation or runoff.
...
`<` returns TRUE if value is less than other value
`<=` returns TRUE if value is less than or equal to other value
`>` returns TRUE if value is greater than other value
`>=` returns TRUE if value is greater than or equal to other value
`&` checks multiple conditions
`|` checks if one or both are TRUE
...
NTL.phys.data.TuesdayLake.1990to1994 <- filter(
  NTL.phys.data, lakename == "Tuesday Lake" | 
    year4 >= 1990 & year4 <= 1994)

```
Question: Why don't we filter using row numbers?

> Answer: We don't filter using row numbers since this may hide certain points of data we need
...
It's important to separate the wrangling code and the analysis code in order to go back and see the raw data we worked with and what we changed before performing analysis
