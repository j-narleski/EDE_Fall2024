...
```{r Ozone.2018, echo=FALSE, warning=FALSE, fig.align='center', fig.width=7, fig.height=4, fig.cap='Ozone Levels by Month of Year 2018'}

# Change "Month" to factor with abbreviation labels
epa_data$Month <- factor(epa_data$Month, levels = 1:12, labels = month.abb)

# Generate boxplot for ozone concentrations during 2018
Ozone.2018 <- epa_data %>%
  filter(Year == 2018) %>%
  ggplot(aes(
    x = Month,
...
```{r, Ozone.2019, echo=FALSE, warning=FALSE, fig.align='center', fig.width=7, fig.height=4,fig.cap='Ozone Levels by Month of Year 2019'}

# Generate boxplot of ozone concentrations for 2019
Ozone.2019 <- epa_data %>%
  filter(Year == 2019) %>%
  ggplot(aes(
    x = Month,
...
## Git Repository
https://github.com/j-narleski/EDE_Fall2024
